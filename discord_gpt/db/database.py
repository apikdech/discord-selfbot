...
class MessageDatabase:
    ...
    async def add_message(self, channel_id: int, message_id: int, author_id: int, content: str):
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
    ...
    async def get_channel_history(self, channel_id: int, limit: int = 150) -> List[Dict[str, Any]]:
        async with aiosqlite.connect(self.db_path) as db:
