...
class DiscordGPTClient():
    ...
    async def generate_gpt_response(self, channel_id: int, prompt: str) -> str:
        ...
        messages = [
            {"role": "system", "content": "You are a superior human. Keep responses short and simple. Do not behave like a bot. "}
        ]
        
        # Add message history as context
        for msg in reversed(history[-100:]):  # Use last 100 messages as context
            messages.append({"role": "user", "content": f"{msg['author_id']}: {msg['content']}"})
    ...
    async def add_message_history(self, channel_id: int, message_id: int, author_id: int, content: str):
